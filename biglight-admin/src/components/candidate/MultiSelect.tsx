...
export default function MultiSelect({ label, options, value, onChange }: { label: string; options: string[]; value: string[]; onChange: (v: string[]) => void }) {
  ...
  return (
    <div ref={ref} className="relative">
      <button type="button" onClick={() => setOpen((o) => !o)} className="flex w-full items-center justify-between rounded-xl border border-bl-line bg-white px-3 py-3 text-sm font-semibold">
        <span className={value.length ? "text-ink" : "text-bl-gray2"}>{value.length ? `${label}（${value.length}）` : label}</span>
        <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M6 9l6 6 6-6" /></svg>
      </button>
      {open && (
        <div className="absolute z-40 mt-1 max-h-64 w-full overflow-y-auto rounded-xl border border-bl-line bg-white p-1 shadow-xl">
          {options.map((o) => (
