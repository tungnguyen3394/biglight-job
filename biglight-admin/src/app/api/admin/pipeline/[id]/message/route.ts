...
export async function POST(req: Request, { params }: { params: { id: string } }) {
  ...
  const translated = await translate(text, "vi", "ja");
  const role: MessageSender = g.level === "ADMIN" ? "ADMIN" : "STAFF";

  await prisma.message.create({
    data: { conversationId: conv.id, senderId: g.user.id, senderRole: role, originalLanguage: "ja", originalText: text, translatedText: translated, translatedLanguage: "vi", isRead: false },
  });
  await prisma.conversation.update({ where: { id: conv.id }, data: { lastMessage: text, lastMessageAt: new Date(), unreadByCandidate: true, status: "IN_PROGRESS" } });

  const cand = await prisma.candidate.findUnique({ where: { id: app.candidateId }, select: { userId: true } });
  await notify(cand?.userId, { type: "message", title: "新しいメッセージが届きました", body: text.slice(0, 80), link: "/mypage?sec=messages" });
