...
export async function POST(req: Request, { params }: { params: { id: string } }) {
  ...
  await prisma.conversation.update({ where: { id: conv.id }, data: { lastMessage: jaText, lastMessageAt: new Date(), unreadByCandidate: true, status: "IN_PROGRESS", aiPausedUntil: new Date(Date.now() + 24 * 3600 * 1000) } });
